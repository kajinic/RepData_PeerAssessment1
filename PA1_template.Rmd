...
The mean number of steps taken each day is `r mean.total.steps`.  
...
The median number of steps taken each day is `r median.total.steps`.  
...
The interval `r max.steps$interval` contains, on average, the maximum number of steps.  

The number of steps in `r max.steps$interval` interval is `r max.steps$steps`.  
...
The number of missing values in the dataset is `r missing.data`.  
...
After the missing values are imputed, the mean number of steps taken each day is `r mean.new.dailysum`.  
...
After the missing values are imputed, the median number of steps taken each day is `r median.new.dailysum`.  
...
Now we are ready to create a panel plot showing a time series (type = “l”) plot using the 5-minute intervals as the x-axis and the average number of steps taken (averaged across all weekday days or weekend days) as the y-axis.
